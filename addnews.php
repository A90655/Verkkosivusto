    ...
    if(empty($_REQUEST['header'])){
      ...
      if($uploadOk == 0){
        ...
        if (move_uploaded_file($_FILES['imageUpload']["tmp_name"], $target_file)){
          ...
          try {
            $insert_stmt=$connect->prepare("INSERT INTO addnews (header, p1, p2, p3, imagepath) VALUES (:header, :p1, :p2, :p3, :imagepath)");

            if ($insert_stmt->execute(array(':header' => $news_header, ':p1' => $p1, ':p2' => $p2, ':p3' => $p3, ':imagepath' => $target_file))) {
